...
async def add_cron_job(job: JobConfigRequest):
    ...
    try:
        ...
        if existing_job is not None:
            raise ExistingKeyException("A job with this name exists")
        ...
        return {"error": "Failed to add job", "details": str(e)}
...
async def get_cron_jobs():
    try:
        ...
        return {"error": "Failed to get jobs", "details": str(e)} 

async def count_cron_jobs():
    try:
        jobs = redis_client.count_keys_with_prefix(REDIS_JOB_BASE_KEY)
        return jobs
    except Exception as e:
        return {"error": "Failed to get jobs count", "details": str(e)} 